...
import os
import subprocess
import clinical_advisor
import threading
import ecg_visualizer
import importlib
...
# =========================
def open_ai_consult():
    idx = int(timeline.get())
    df = pd.read_csv(CSV_FILE)
    if idx < len(df):
        report = clinical_advisor.get_clinical_report(df.iloc[idx])
        
        # UI Popup for AI report
        top = tk.Toplevel(root)
        top.title("VITAL-CORE | AI Expert Report")
        top.geometry("600x500")
        top.configure(bg="#1e1e1e")
        
        display = tk.Text(top, wrap=tk.WORD, bg="black", fg="#00d4ff", font=("Consolas", 11), padx=15, pady=15)
        display.insert(tk.END, report)
        display.config(state=tk.DISABLED)
        display.pack(fill=tk.BOTH, expand=True)

# def display_report(window, label, report):
#     label.destroy()
#     display = tk.Text(window, wrap=tk.WORD, bg="#000", fg="white", font=("Consolas", 11))
#     display.insert(tk.END, report)
...
def open_ecg_program():
    global last_row
    try:
        # Use a slight delay to allow the Tkinter loop to process the click
        # before Matplotlib opens its own window
        root.after(100, lambda: ecg_visualizer.animate_ecg(start_at_idx=last_row))
    except Exception as e:
        messagebox.showerror("ECG Error", f"Could not launch: {e}")
...
# 1. Play/Pause Button
play_btn = tk.Button(top_bar, text="PAUSE", font=("Arial", 10, "bold"), command=toggle_play, bg="#ffc107", fg="black", width=10, relief="flat")
play_btn.pack(side=tk.LEFT, padx=10)

# 2. ECG Button (Now correctly linked to your synced function)
tk.Button(top_bar, text="ECG VIEW", command=open_ecg_program, bg=ACCENT_COLOR, fg="black", font=("Arial", 10, "bold"), relief="flat").pack(side=tk.LEFT, padx=5)

# 3. Export Button
tk.Button(top_bar, text="EXPORT PNG", command=print_report, bg="#444", fg="white", font=("Arial", 10, "bold"), relief="flat").pack(side=tk.LEFT, padx=5)

# 4. AI Button
tk.Button(top_bar, text="CONSULT AI", command=open_ai_consult, bg="#6f42c1", fg="white", font=("Arial", 10, "bold"), relief="flat").pack(side=tk.LEFT, padx=5)

# 5. Timeline Slider
timeline = tk.Scale(top_bar, from_=0, to=100, orient=tk.HORIZONTAL, length=500, bg=PANEL_COLOR, fg="white", highlightthickness=0, command=lambda x: jump_to(int(x)))
...
# Voice assistance
# =========================
voice_module = importlib.import_module("AI_voice_assistant")
...
def update_graph():
    ...
    if alerts:
        ...
        axs[2,1].text(0.5, 0.5, "SYSTEMS NOMINAL", color="#00ff88", fontsize=14, 
